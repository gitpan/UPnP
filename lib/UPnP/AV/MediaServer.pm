...
sub new {
	my($class) = shift;
	my($this) = {
		$UPnP::AV::MediaServer::_DEVICE => undef,
	};
	bless $this, $class;
}
...
sub setdevice() {
	my($this) = shift;
	if (@_) {
		$this->{$UPnP::AV::MediaServer::_DEVICE} = $_[0];
	}
}

sub getdevice() {
	my($this) = shift;
	$this->{$UPnP::AV::MediaServer::_DEVICE};
}
...
sub browse {
	...
	$dev = $this->getdevice();
	$condir_service = $dev->getservicebyname($UPnP::AV::MediaServer::CONTENTDIRECTORY_SERVICE_TYPE);
...
sub getcontentlist {
	...
	while ($result =~ m/<container(.*?)<\/container>/sgi) {
		$content = $1;
		$container = UPnP::AV::Container->new();
		if ($content =~ m/id=\"(.*?)\"/si) {
			$container->setid($1);
		}
	...
	while ($result =~ m/<item(.*?)<\/item>/sgi) {
		...
		if ($content =~ m/<res[^>]*>(.*?)<\/res>/si) {
			$item->seturl(UPnP::HTTP::xmldecode($1));
		}
...
UPnP::AV::MediaServer - Perl extension for UPnP.
...
=item B<new> - create new UPnP::AV::MediaServer.

    $mservier = UPnP::AV::MediaServer();
...
Browse the content directory and return the action response, L<UPnP::ActionResponse>.
...
Browse the content directory and return the content list. Please see L<UPnP::AV::Content>, L<UPnP::AV::Item> and L<UPnP::AV::Container>.
