...
sub new {
	...
	my($this) = {
		$UPnP::AV::Content::_ID => '',
		$UPnP::AV::Content::_TITLE => '',
		$UPnP::AV::Content::_DATE => '',
	};
	bless $this, $class;
}
...
sub setid() {
	my($this) = shift;
	if (@_) {
		$this->{$UPnP::AV::Content::_ID} = $_[0];
	}
}

sub getid() {
	my($this) = shift;
	$this->{$UPnP::AV::Content::_ID};
}
...
sub settitle() {
	my($this) = shift;
	if (@_) {
		$this->{$UPnP::AV::Content::_TITLE} = $_[0];
	}
}

sub gettitle() {
	my($this) = shift;
	$this->{$UPnP::AV::Content::_TITLE};
}
...
sub setdate() {
	my($this) = shift;
	if (@_) {
		$this->{$UPnP::AV::Content::_DATE} = $_[0];
	}
}

sub getdate() {
	my($this) = shift;
	$this->{$UPnP::AV::Content::_DATE};
}
...
The package is a extention UPnP/AV media server, and a super class of L<UPnP::AV::Container> and L<UPnP::AV::Item>.
